...
namespace blueth::container{
	template<typename T, std::size_t buffer_size> class StackArray{
		...
		public:
			...
			StackArray<T, buffer_size>& operator=(const StackArray<T, buffer_size>&) = delete;
	...
	template<typename T, std::size_t buffer_size> inline StackArray<T, buffer_size>::operator T*() noexcept {
